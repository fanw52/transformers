...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.train_file.endswith(".csv"):
            ...
            ds = load_dataset("csv", data_files=data_files, cache_dir=model_args.cache_dir)
        else:
    ...
    metric = datasets.load_metric("./metric/accuracy.py")
    ...
    def val_transforms(example_batch):
        ...
        example_batch["pixel_values"] = [_train_transforms(Image.open(example["path"]).convert("RGB")) for example in
                                         example_batch["image"]]
        example_batch["labels"] = [int(label2id[l]) for l in example_batch["labels"]]
        return example_batch
